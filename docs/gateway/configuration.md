- [agents.list[].identity](#agents-list%5B%5D-identity)
- [Group chat mention gating (agents.list[].groupChat + messages.groupChat)](#group-chat-mention-gating-agents-list%5B%5D-groupchat-%2B-messages-groupchat)
...
VariableDescriptionExample`{model}`Short model name`claude-opus-4-5`, `gpt-4o``{modelFull}`Full model identifier`anthropic/claude-opus-4-5``{provider}`Provider name`anthropic`, `openai``{thinkingLevel}`Current thinking level`high`, `low`, `off``{identity.name}`Agent identity name(same as `"auto"` mode)
...
*Next: [Agent Runtime](/concepts/agent)* 🦞[Environment Variables](/environment)